...
function login() {
    ...
    if (username === 'admin' && password === 'admin123') {
        ...
        document.getElementById('adminTeamControls') && (document.getElementById('adminTeamControls').style.display = 'block');
        closeLoginModal();
        loadGallery();
        loadEvents();
        loadTeamMembers();
        ...
        document.getElementById('adminTeamControls') && (document.getElementById('adminTeamControls').style.display = 'none');
        closeLoginModal();
        loadGallery();
        loadEvents();
        loadTeamMembers();
...
function submitForm(event) {
    ...
    const webhookUrl = 'https://discord.com/api/webhooks/1347657602657685594/yMy8AW-B2AwUCG3Sr0De0g-A-kcoyo90HtjVdsNKgkB8H8ou3FSUiHu_mtskTCYfYJPm';
...
function sendToDiscord(event) {
    ...
    .then(response => {
        if (response.ok) {
            status.textContent = 'Сообщение успешно отправлено в Discord!';
            form.reset();
